...
from app.parsers.three_pass_parser import groq_request_with_backoff, _is_text_garbage
...
def _quality_score(text):
    ...
    weird = stripped.count("�")
    return (alpha * 2) + digits + (words * 4) - (pipes * 3) - (underscores * 2) - (weird * 20)
...
def _ocr_page_with_fallback(page):
    ...
    for config in ("--oem 3 --psm 6", "--oem 3 --psm 11"):
        try:
            ...
            candidates.append((text, config))
        except Exception:
            continue
    if not candidates:
        return ""
    best_text, _ = max(candidates, key=lambda item: _quality_score(item[0]))
    return best_text
...
def pass1_extract_application_layout(pdf_path):
    ...
    Uses digital text extraction when good; OCR fallback when weak/garbled.
    """
    pages = []
    with pdfplumber.open(pdf_path) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            if use_ocr:
                ...
                # If extraction quality is low, compare against OCR and keep better text.
                ocr_text = _ocr_page_with_fallback(page)
                text = ocr_text if _quality_score(ocr_text) > _quality_score(extracted_text) else extracted_text
